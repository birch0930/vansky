...
<%! 
	...
	static int from = 1 ;
	static int to = from + PAGE_BAR_SIZE-1;  
	%>
<%
	...
	String key = "";
 	...
 	if(action =="search"){	
	 	 key = (String)request.getAttribute("keyword");	
	 		...
	 		if(key !=null){
	 		list = service.searchNewsBykeyWord(key,currentPage,PAGESIZE);
	 		totalRecords = service.getTotalSerachNews(key);
 ...
 <form  class="pure-form">
   <fieldset>
      <legend>新闻发布</legend>
		...
		<input id="search_news" name="search_news" onkeydown="enterkeyprevent()"  placeholder="输入关键字" type="text"  maxlength="128" value="<%=key  %>" />
	...
	<% }%>
		...
		<div>		
		 <ul class="pagination pagination-sm" >
			<li><a href="javascript:void(0)" onclick="javascript:turnpage(-1,'<%= action %>')">&laquo;</a></li>
			<%     	
			...
			%>
				...
				<li id="page<%= i %>"><a href="javascript:void(0)"   <%= (currentPage == i )? "style='color:#fff; background-color: #428bca;'":""%>
				 onclick="javascript:gotopage(<%= i %>,'<%= action %>')"><%= i %></a></li>
			...
			<li><a href="javascript:void(0)" onclick="javascript:turnpage(1,'<%= action %>')">&raquo;</a></li>
		</ul>
